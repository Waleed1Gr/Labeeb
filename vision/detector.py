...
# ! completely removed this to use the camera.py instead, this might come back later if needed, for now it's commented out.
...
# def detect_from_image_bytes(img_bytes):
#     """Run detection on image bytes (from Pi camera)."""
...
#         detections.append(
#             {"box": [int(x) for x in box], "confidence": float(conf), "class": int(cls)}
#         )
#     return detections
...
# phone_put_down_time = (
#     {}
# )  # Track when phone was put down  # Track if warning was given for current phone session
...
#             aspect_ratio = max(phone_height, phone_width) / min(
...
#             if (
#                 px1 - 100 <= phone_center_x <= px2 + 100
#                 and py1 - 100 <= phone_center_y <= py2 + 100
#             ):
...
#             has_phone, phone_conf, phone_box = check_phone_detection(
...
#             else:
#                 if (
#                     person_id in phone_start_times
#                     and phone_start_times[person_id] is not None
#                 ):
...
#                     phone_put_down_time[person_id] = (
#                         current_time  # Record when phone was put down
#                     )
...
# phone_percentage = (
#     (total_phone_time / total_session_time * 100) if total_session_time > 0 else 0
# )
